...
const ChatInterface = () => {
  ...
  const handleSend = async (textOverride = "") => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Connection to Archive failed.",
          sources: [],
        },
      ]);
    } finally {
  ...
  return (
    <section
      id="chatbot"
      className="py-32 px-6 bg-[#EEF4ED] relative overflow-hidden"
    >
        ...
        <div className="w-full h-[75vh] flex flex-col bg-white border border-[#0B2545]/10 shadow-2xl relative overflow-hidden">
          ...
          <div className="p-6 border-b border-[#0B2545]/5 flex justify-between items-center bg-white z-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-[#0B2545] font-black text-sm uppercase tracking-tighter">
                  Maaz's virtual assistant
                </h3>
          ...
          <div
            ref={scrollRef}
            className="flex-1 overflow-y-auto p-8 space-y-10 scrollbar-hide bg-[#F9FAFB]/30"
          >
            {messages.length === 0 && (
            ...
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] space-y-2 ${msg.role === "user" ? "text-right" : ""}`}
                >
                  <div
                    className={`p-6 border ${msg.role === "user" ? "bg-[#0B2545] text-white border-transparent" : "bg-white text-[#0B2545] border-[#0B2545]/10 shadow-sm"}`}
                  >
                  ...
                  {msg.sources && (
                    <div className="flex flex-wrap gap-2 justify-start mt-2">
                      {msg.sources.map((s) => (
                        <span
                          key={s}
                          className="text-[8px] font-mono bg-[#0B2545]/5 px-2 py-1 text-[#0B2545]/40 border border-[#0B2545]/10 uppercase"
                        >
                          REF//{s}
                        </span>
                      ))}
            ...
            {isLoading && (
              <div className="text-[10px] font-mono text-[#134074]/30 uppercase animate-pulse">
                Syncing with Vector Archive...
              </div>
            )}
          </div>
